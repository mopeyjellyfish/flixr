...
export const api = {
  ...
  playbackHeartbeat: (sessionID: string, positionMs: number, ended = false) => request<{ expires_at: number }>(`/playback/sessions/${encodeURIComponent(sessionID)}/heartbeat`, { method: 'POST', body: JSON.stringify({ position_ms: positionMs, observed_at: Date.now(), ended }) }),
  playbackSeek: (sessionID: string, positionMs: number) => request<PlaybackPlan>(`/playback/sessions/${encodeURIComponent(sessionID)}/seek`, { method: 'POST', body: JSON.stringify({ position_ms: positionMs, observed_at: Date.now() }) }),
