...
export const api = {
  ...
  playbackQuality: (sessionID: string, positionMs: number, observation: number, capabilities: PlaybackCapabilities, quality: import('../core/api').PlaybackQuality) => request<PlaybackPlan>(`/playback/sessions/${encodeURIComponent(sessionID)}/quality`, { method: 'POST', body: JSON.stringify({ position_ms: positionMs, observation, capabilities, quality }) }),
