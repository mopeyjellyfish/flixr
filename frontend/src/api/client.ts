...
export const api = {
  ...
  playbackHandoff: (url: string, attached: boolean) => request<{ attached: boolean }>(url.startsWith('/api/v1') ? url.slice('/api/v1'.length) : url, { method: 'POST', body: JSON.stringify({ attached }) }),
