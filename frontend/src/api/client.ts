...
export const api = {
  ...
  episodeOrderSeries: (query = '', offset = 0, limit = 50) => request<{ series: EpisodeOrderSeries[]; total: number; next_offset?: number }>(`/owner/episode-orders?${new URLSearchParams({ q: query, offset: String(offset), limit: String(limit) })}`),
  episodeOrder: (id: string) => request<EpisodeOrderDetail>(`/owner/episode-orders/${encodeURIComponent(id)}`),
  saveEpisodeOrder: (id: string, order: EpisodeOrderKind, revision: number, entries: EpisodeOrderEntry[]) => request<EpisodeOrderDetail>(`/owner/episode-orders/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify({ order, revision, entries }) }),
  episodeOrderGroups: (id: string) => request<{ groups: EpisodeOrderGroup[] }>(`/owner/episode-orders/${encodeURIComponent(id)}/groups`),
  previewEpisodeOrder: (id: string, groupID: string) => request<EpisodeOrderDetail>(`/owner/episode-orders/${encodeURIComponent(id)}/preview`, { method: 'POST', body: JSON.stringify({ group_id: groupID }) }),
