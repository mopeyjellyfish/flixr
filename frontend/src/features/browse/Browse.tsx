...
function initialVersion(versions?: MediaVersion[]) { return versions?.find((version) => version.selected)?.id ?? versions?.find((version) => version.available)?.id ?? versions?.[0]?.id; }
function VersionSelect({ versions, value, onChange, label }: { versions: MediaVersion[]; value?: string; onChange: (id: string) => void; label: string }) {
  return <label className="version-select">{label}<select aria-label={label} value={value ?? ''} onChange={(event) => onChange(event.target.value)}>{versions.map((version) => <option key={version.id} value={version.id} disabled={!version.available && version.id !== value}>{version.label}{version.available ? '' : ' · unavailable'}</option>)}</select></label>;
}
