...
function RatingControl({ catalogID }: { catalogID: string }) { const [value,setValue]=useState(0); const [error,setError]=useState(''); useEffect(()=>{let active=true;api.rating(catalogID).then(({rating})=>{if(active)setValue(rating?.value??0);}).catch(()=>{if(active)setError('Rating unavailable.');});return()=>{active=false;};},[catalogID]); const save=async(next:number)=>{try{setError('');if(next===0)await api.deleteRating(catalogID);else await api.setRating(catalogID,next);setValue(next);}catch{setError('Flixr could not save your rating.');}};return <label className="rating-control">Your rating<select aria-label="Your rating" value={value} onChange={(event)=>void save(Number(event.target.value))}><option value={0}>Not rated</option>{[1,2,3,4,5].map(stars=><option key={stars} value={stars}>{stars} star{stars===1?'':'s'}</option>)}</select>{error&&<span>{error}</span>}</label>; }
