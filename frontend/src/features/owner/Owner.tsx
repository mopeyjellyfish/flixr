...
export function Owner({ onLogout, onBrowse }: OwnerProps) {
  ...
  const saveScanPolicy=async(policy:ScanPolicy)=>{try{const result=await api.saveScanPolicy(policy.library_id,policy);setScanPolicies((current)=>({...current,[policy.library_id]:result.policy}));setNotice('Scan schedule saved.');}catch(error){setNotice(error instanceof ApiError?error.message:'Unable to save scan schedule.');}};
  ...
  const retryScanJob=async(job:ScanJob)=>{try{const result=await api.retryScanJob(job.id,(job.files??[]).filter((file)=>file.retryable));setScanJobs((current)=>[result.job,...current]);setNotice('Failed files queued for retry.');}catch(error){setNotice(error instanceof ApiError?error.message:'Unable to retry files.');}};
...
function ScanSchedules({ libraries, policies, jobs, locked, onSave, onRun, onCancel, onRetry }: { libraries: Library[]; policies: Record<string, ScanPolicy>; jobs: ScanJob[]; locked: boolean; onSave: (policy: ScanPolicy) => Promise<void>; onRun: (libraryID: string) => Promise<void>; onCancel: (id: string) => Promise<void>; onRetry: (job: ScanJob) => Promise<void> }) {
  return <section className="owner-panel" aria-labelledby="scan-schedules-title">
    ...
    {locked && <p className="muted">Managed by environment through FLIXR_SCAN_SCHEDULE.</p>}
    ...
    {jobs.length===0?<p className="muted">No scheduled or manual scan jobs yet.</p>:<div className="stack">{jobs.map((job)=>{
      const active=job.status==='queued'||job.status==='running';const retryable=(job.files??[]).some((file)=>file.retryable)||job.status==='failed'||job.status==='interrupted';
      return <article className="setting-row" key={job.id}><div><strong>{libraries.find((library)=>library.id===job.library_id)?.name??'Library'} · {job.status}</strong><p className="muted">{job.trigger} · {job.scanned} scanned · {job.skipped} unchanged · {job.failed} failed{job.total!==undefined?` · ${job.total} total`:''}{job.started_at?` · ${formatElapsed(job.started_at,job.finished_at)} elapsed`:''}</p>{job.message&&<p role="alert">{job.message}</p>}{job.files?.map((file)=><p className="muted" key={`${file.location_id}:${file.relative_path}`}>{file.relative_path}: {file.message??file.outcome}</p>)}{job.status==='running'&&<ProgressBar value={null} label="Library scan" pendingLabel="Scanning…" />}</div><div className="button-row">{active&&<button type="button" className="secondary" onClick={()=>void onCancel(job.id)}>Cancel</button>}{retryable&&<button type="button" className="secondary" onClick={()=>void onRetry(job)}>Retry failed files</button>}</div></article>;
    })}</div>}
  </section>;
}
