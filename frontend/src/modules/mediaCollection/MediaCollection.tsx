...
function PosterCard({ item, onOpen, style, onKeyDown }: { item: ViewerItem; onOpen: Open; style?: CSSProperties; onKeyDown?: KeyboardEventHandler<HTMLButtonElement> }) {
  return <button data-testid={`card-${item.id}`} data-catalog-id={item.id} data-card className="card" style={style} onKeyDown={onKeyDown} onClick={(event) => onOpen(item, event.currentTarget)}>{item.poster ? <Artwork key={item.poster} src={item.poster} width={style?.width as number | undefined} /> : <span className="card-artwork" style={tonalStyle(item.title)} aria-hidden="true" />}<span className="card-scrim" aria-hidden="true" /><span className="card-kind">{kindLabel(item)}</span><strong>{item.title}</strong>{item.year && <small>{item.year}</small>}{item.demo && <small className="card-demo">Preview</small>}<span className="card-open" aria-hidden="true">↗</span></button>;
}
