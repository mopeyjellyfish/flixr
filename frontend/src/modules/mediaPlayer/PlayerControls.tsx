...
function Icon({ name }: { name: 'play' | 'pause' | 'back' | 'forward' | 'volume' | 'muted' | 'fullscreen' | 'pip' }) {
  const paths = { play: 'M8 5v14l11-7z', pause: 'M7 5v14M17 5v14', back: 'M10 5 3 12l7 7M20 5l-7 7 7 7', forward: 'm4 5 7 7-7 7m10-14 7 7-7 7', volume: 'M3 9h4l5-4v14l-5-4H3zM16 8c3 2 3 6 0 8m3-11c5 4 5 10 0 14', muted: 'M3 9h4l5-4v14l-5-4H3zM16 9l6 6m0-6-6 6', fullscreen: 'M9 3H3v6m12-6h6v6M3 15v6h6m6 0h6v-6', pip: 'M3 4h18v16H3zM12 11h7v7h-7z' };
...
export function PlayerControls({ playing, durationMS, positionMS, disabled, seeking = false, onSeek, onToggle, onPrevious, onNext, video, stage, chapters = [], sessionID, children }: Props) {
  ...
  useEffect(() => {
    ...
    const sync = () => { if (element) { setVolume(element.volume); setMuted(element.muted); setSpeed(element.playbackRate); } };
