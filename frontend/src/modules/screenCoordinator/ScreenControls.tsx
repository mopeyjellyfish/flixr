...
export function ScreenReadiness({ catalogID }: { catalogID?: string }) {
  ...
  const screens = state.status === 'ready' ? state.screens : [];
  const choose = async (screen: ScreenPresence) => {
    await screenCoordinator.connect(screen);
    if (catalogID) screenCoordinator.command({ version: 1, type: 'play', catalog_id: catalogID, position_ms: 0 });
    chooser.current?.close();
  };
  return <section className="screen-readiness" aria-label="Local screens">
    <p role="status">{state.status === 'connected' ? `Playing on ${state.screen.name}` : state.status === 'receiving' ? `${state.screen.name} is ready` : state.status === 'disconnected' || state.status === 'error' ? state.message : screens.length ? `${screens.length} local screen${screens.length === 1 ? '' : 's'} ready` : 'No other Flixr screens ready'}</p>
