...
export type ScanPolicy = { library_id: string; enabled: boolean; schedule_kind: 'interval' | 'daily'; interval_seconds: number; local_time: string; timezone: string; next_run_at?: number; last_success_at?: number; exclusions: string[] };
export type ScanJobFile = { location_id: string; relative_path: string; outcome: string; error_code?: string; message?: string; retryable: boolean };
export type ScanJob = { id: string; library_id: string; trigger: 'manual' | 'schedule' | 'startup' | 'retry'; status: 'queued' | 'running' | 'succeeded' | 'partial' | 'failed' | 'cancelled' | 'interrupted'; queued_at: number; started_at?: number; finished_at?: number; attempt: number; total?: number; scanned: number; skipped: number; failed: number; unmatched: number; message?: string; files?: ScanJobFile[] };
