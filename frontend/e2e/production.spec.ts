...
test('built binary completes setup, scan, profile, browse, and detail flow', async ({ page, browser }, testInfo) => {
  ...
  await page.screenshot({ path: testInfo.outputPath('production-setup-choice-desktop.png'), fullPage: true });
  await page.getByRole('button', { name: /start fresh/i }).click();
  await expect(page.getByRole('heading', { name: /bring your libraries home/i })).toBeVisible();
  await page.reload();
  await expect(page.getByRole('heading', { name: /bring your libraries home/i })).toBeVisible();
  await expect(page.getByLabel(/films library/i)).toHaveValue('');
  await page.getByLabel(/tv library/i).fill(tvRoot);
  await page.getByLabel(/films library/i).fill('/media/not-mounted');
  await page.getByRole('button', { name: /recheck folders and tools/i }).click();
  const diagnostics = page.getByRole('region', { name: /setup diagnostics/i });
  const missingMount = diagnostics.locator('article.needs-attention').filter({ hasText: '/media/not-mounted' });
  await expect(missingMount).toContainText(/compose/i);
  await page.screenshot({ path: testInfo.outputPath('production-setup-diagnostics-failure-desktop.png'), fullPage: true });
  await page.getByLabel(/films library/i).fill(filmsRoot);
  await page.getByRole('button', { name: /recheck folders and tools/i }).click();
  await expect(diagnostics.getByText(filmsRoot, { exact: true })).toBeVisible();
  await expect(diagnostics.getByText(tvRoot, { exact: true })).toBeVisible();
  await expect(diagnostics.locator('article.needs-attention')).toHaveCount(0);
  await page.screenshot({ path: testInfo.outputPath('production-setup-diagnostics-desktop.png'), fullPage: true });
  ...
  await expect(continueWatching.getByRole('button', { name: /film blue horizon 2026/i })).toBeVisible();
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBeTruthy();
  const curationAxe = await new AxeBuilder({ page }).analyze();
  expect(curationAxe.violations.filter((violation) => violation.impact === 'serious' || violation.impact === 'critical')).toEqual([]);
  await continueWatching.getByRole('button', { name: 'Hide Blue Horizon 2026 from Continue Watching' }).click();
  await expect(page.getByRole('status')).toContainText('Blue Horizon 2026 hidden from Continue Watching');
  await expect(continueWatching.getByRole('button', { name: /film blue horizon 2026/i })).toHaveCount(0);
  await page.screenshot({ path: testInfo.outputPath('production-continue-watching-hidden-phone.png'), fullPage: true });
  ...
  await expect(continueWatching.getByRole('button', { name: /film blue horizon 2026/i })).toHaveCount(0);
  await page.getByRole('region', { name: 'New' }).getByRole('button', { name: /film blue horizon 2026/i }).click();
  await page.getByRole('button', { name: 'Restore to Continue Watching' }).click();
  await expect(continueWatching.getByRole('button', { name: /film blue horizon 2026/i })).toBeVisible();
