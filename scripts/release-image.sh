...
if [[ "$mode" == prepare ]]; then
	[[ -n "${FLIXR_TMDB_APPLICATION_TOKEN:-}" ]] || { echo 'FlixR TMDB application credential is required for an official image' >&2; exit 1; }
	...
	if ! printf 'header = "Authorization: Bearer %s"\n' "$FLIXR_TMDB_APPLICATION_TOKEN" \
		...
		echo 'FlixR TMDB application credential did not validate; refusing official image publication' >&2
		exit 1
	fi
	...
	docker buildx build --platform linux/amd64,linux/arm64 --push --no-cache-filter runtime \
		...
		--secret id=tmdb_application_token,env=FLIXR_TMDB_APPLICATION_TOKEN \
	...
	for arch in amd64 arm64; do
		FLIXR_SMOKE_PLATFORM="linux/$arch" FLIXR_EXPECT_APPLICATION_METADATA=1 bash scripts/image-smoke.sh "$image:v$version" "$version"
	done
else
