...
func TestFFmpegCommandUsesNoShellAndRejectsNonLoopbackInput(t *testing.T) {
	...
	for _, exact := range []string{"-readrate 1", "-readrate_initial_burst 4", "-readrate_catchup 8", "-profile:v high", "-level:v 4.0", "-pix_fmt yuv420p", "-r 30", "-b:v 5000000", "-maxrate 5000000", "-bufsize 10000000", "-force_key_frames expr:gte(t,n_forced*2)", "-c:a aac", "-ac 2", "-ar 48000", "-b:a 128000", "-hls_time 2", "-master_pl_name master.m3u8"} {
	...
	if strings.Count(external, "-ss 23.000") != 2 || strings.Count(external, "-readrate_catchup 8") != 2 || !strings.Contains(external, "-map 1:0") {
...
func TestFFmpegCommandFallsBackToPreCatchupPacing(t *testing.T) {
	args := []string{"-readrate", "1", "-readrate_initial_burst", "4", "-readrate_catchup", "8", "-i", "input"}
	...
	if strings.Join(args, " ") != "-readrate 1 -readrate_initial_burst 4 -readrate_catchup 8 -i input" {
