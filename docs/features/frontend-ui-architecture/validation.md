...
| --- | --- | --- | --- |
| Rail card width and JavaScript stride disagreed | CSS and React both owned geometry | `MediaCollection` measures the container and owns card width, 16px gap, stride, and rail height | MediaCollection geometry/focus tests; four mocked viewports |
...
- Mocked Playwright matrix — 27 tests passed in Chromium, Firefox, and WebKit, including a real-browser 1,000-item virtualized grid scroll. During implementation, the strict router exposed an undeclared catalog request after Player exit; the route was declared before the final green matrix.
