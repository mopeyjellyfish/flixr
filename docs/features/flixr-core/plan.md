...
- Compose concrete `Catalog`, `Household`, `Playback`, `Screens`, and `Web` values in `backend/main.go`; it contains lifecycle and wiring, not business behavior. Consumer-owned interfaces are permitted only for accepted real alternatives: metadata lookup, process execution in tests, and the user-required Afero filesystem boundary for application-owned file I/O.
...
- Use Go's standard library first, `modernc.org/sqlite` for SQLite, `github.com/gofrs/flock` for cross-platform advisory data/segment locks, `github.com/spf13/afero` for the requested consistent application-owned filesystem I/O and deterministic in-memory filesystem tests, `github.com/coder/websocket` for context-aware WebSockets, `golang.org/x/crypto/argon2` for memory-hard owner/PIN hashing, `golang.org/x/sync/errgroup` for bounded cancellable scan work, and `github.com/stretchr/testify` assertions without suites. Keep `os.Root` for symlink-safe catalog containment and real OS paths at the FFmpeg process boundary. Use a dedicated server-derived WebSocket context rather than the upgraded request context; keep same-origin rejection or an explicit trusted-origin allowlist, and treat I/O context cancellation as connection-fatal. Record why each non-standard dependency is required.
...
- Required command definitions are `(cd backend && go test ./...)`, `(cd backend && go test -race ./...)`, and `(cd backend && go vet ./...)`; `npm --prefix frontend run lint`, `npm --prefix frontend run typecheck`, `npm --prefix frontend run test -- --run`, `npm --prefix frontend run build`, and `npm --prefix frontend run test:bundle`; and `npm --prefix frontend run test:e2e -- --project=chromium`, `--project=firefox`, or `--project=webkit` for the named browser matrix. The required media job runs `(cd backend && FLIXR_REQUIRE_MEDIA_INTEGRATION=1 go test -tags=media_integration ./...)` after installing FFmpeg/ffprobe and treats a required skip as failure.
...
- HTTP listeners keep explicit header-read, request-read where applicable, idle, and shutdown bounds. They intentionally omit a total `WriteTimeout`: direct media and loopback FFmpeg inputs can stream longer than any fixed response-wide deadline. LAN scope, authenticated media authority, playback leases, bounded generation processes, and ten-second shutdown bound the accepted release-one risk from a slow reader.
...
## [x] 004 — Capability-planned direct playback and cross-client resume
...
## [x] 005 — Bounded remux, transcode, seek generations, and media gate
