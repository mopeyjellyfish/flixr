...
Scheduled and manual library work uses the durable owner-only job API. List or
queue jobs with `GET` or `POST /api/v1/owner/scan/jobs`, inspect or cancel one at
`/api/v1/owner/scan/jobs/{id}`, and retry its retryable files with
